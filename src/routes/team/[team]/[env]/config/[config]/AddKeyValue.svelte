<script lang="ts">
	...
	const addConfigValue = async () => {
		...
		// Reload config after adding new key
		if (onSuccess) {
			await onSuccess();
		}
	};
...
<Modal bind:open onclose={reset} width="medium">
	...
	{#snippet footer()}
		{#if key === '' || value === '' || validKey(key) !== ''}
			<Button variant="primary" size="small" onclick={addConfigValue} disabled={true}>Add</Button>
		{:else}
			<Button variant="primary" size="small" onclick={addConfigValue}>Add</Button>
		{/if}
