...
<div class="wrapper">
	{#if $ResourceUtilizationForApp.data}
		{const utilization = $derived($ResourceUtilizationForApp.data.team.environment.application.utilization)}
		<div class="grid">
						...
						<Tooltip.Root>
							{#snippet children({ data })}
								{#if data.annotation}
									...
									{/each}
								{:else}
									{const request = $derived(utilization.requested_cpu_series.find(
										(d) => d.timestamp.getTime() === context.x(data).getTime()
									)?.value)}
									{const limit = $derived(utilization.limit_cpu_series.find(
										(d) => d.timestamp.getTime() === context.x(data).getTime()
									)?.value)}
						...
						<Tooltip.Root>
							{#snippet children({ data })}
								{#if data.annotation}
									...
									{/each}
								{:else}
									{const request = $derived(utilization.requested_memory_series.find(
										(d) => d.timestamp.getTime() === context.x(data).getTime()
									)?.value)}
									{const limit = $derived(utilization.limit_memory_series.find(
										(d) => d.timestamp.getTime() === context.x(data).getTime()
									)?.value)}
									<Tooltip.Header>{format(context.x(data), 'dd/MM/yyyy HH:mm')}</Tooltip.Header>
									<Tooltip.List>
