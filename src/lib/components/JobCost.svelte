<script lang="ts">
	...
	import type { AggregatedCostJobVariables } from './$houdini';

	export const _AggregatedCostJobVariables: AggregatedCostJobVariables = () => {
		return { job: job, environment: environment, team: team };
	};

	const costQuery = graphql(`
		query AggregatedCostJob($team: Slug!, $environment: String!, $job: String!) @load {
			team(slug: $team) {
...
{:else if $costQuery.data !== null}
	{@const cost = $costQuery.data.team.environment.job.cost}
	<div>
