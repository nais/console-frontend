	...
	spec: {
		...
		azure: {
			application: {
				...
				enabled: true,
				replyURLs: ['https://myapplication.nav.no/oauth2/callback'],
				singlePageApplication: true,
				tenant: 'nav.no'
			},
			sidecar: {
				autoLogin: true,
				autoLoginIgnorePaths: ['/path', '/internal/*'],
				enabled: true,
				errorPath: '/error',
				resources: {
					limits: {
		...
		command: ['/app/myapplication', '--param', 'value', '--other-param', 'other-value'],
		env: [
			{
				name: 'MY_CUSTOM_VAR',
				value: 'some_value'
		...
		idporten: {
			...
			postLogoutRedirectURIs: ['https://www.nav.no'],
		...
		ingresses: ['https://myapplication.nav.no'],
		kafka: {
			pool: 'nav-dev',
			streams: true
		},
		...
		maskinporten: {
			...
			scopes: {
				...
				exposes: [
					{
						allowedIntegrations: ['maskinporten'],
						atMaxAge: 30,
						consumers: [
							{
								name: 'KST',
